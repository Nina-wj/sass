...
*     前台学习测试页面公共部分
* --------------------------------------------------------------------------*/
KISSY.add('page/common/exam-answer',function(S,get,exit,click,index){
	PW.namespace('page.common.ExamAnswer');
	PW.page.common.ExamAnswer = function(param,IO){
		new get(param,IO);
		new exit(param);
		new click(param);
		new index();
	...
	requires:['ExamAnswer/get','ExamAnswer/exit','ExamAnswer/click','ExamAnswer/index']
});
/*--------------------------------获取答案------------------------------*/
KISSY.add('ExamAnswer/get',function(S,Juicer){
	var
	...
	S.augment(get,{
		...
		_getNextQuestion:function(){
			...
			window.location.href = this.opts.nextUrl + '/' + answer + '/' + questionId;
		},
		/**
		...
		_submitError:function(){
			...
			that.ExamIO.submitError(data,function(rs){
				if(rs.code != 0){
					PW.page.Util.dialog.alert('<h1>'+flag+'</h1>');
				}
			});
		},
		/**
	...
	requires:['mod/juicer','core','mod/dialog']
});
/*--------------------------------退出------------------------------*/
KISSY.add('ExamAnswer/exit',function(S){
	var
...
/*--------------------------------点击非白色地方报错------------------------------*/
KISSY.add('ExamAnswer/click',function(S){
	var
...
/*********************将页面导航页面操作加入到页面****************************/
KISSY.add('ExamAnswer/index',function(S){
	function index(){
		this._init();
	}
	S.augment(index,{
