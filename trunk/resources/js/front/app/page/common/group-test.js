/*-----------------------------------------------------------------------------
* @Description:     前台学习套题测试答题页面操作公共部分
* @Version:         1.0.0
...
* --------------------------------------------------------------------------*/
KISSY.add('page/common/group-test',function(S,ExamManage,exit,click,timer,index){
	PW.namespace('page.common.GroupTest');
	PW.page.common.GroupTest = function(param,IO){
	...
	requires:['groupExamTest/getAndNum','groupExamTest/exit','groupExamTest/click','groupExamTest/timer','groupExamTest/index']
});
/*--------------------------------获取题目------------------------------*/
KISSY.add('groupExamTest/getAndNum',function(S,Juicer){
	var
	...
	S.augment(ExamManage,{
			...
			that.ExamIO.validQuestion(data,function(rs){
				...
				if(rs.code == 0){
					PW.page.Util.dialog.alert('<h1>总共消费豆数为：'+rs.data.howMuchBalanceCounts+'个</h1>',function(){
						window.location.href = that.opts.submitUrl;
					});
				//当用户豆数不足时，提示信息并跳转到充值页面
				}else if(rs.code == 2){
					PW.page.Util.dialog.alert('<h1>总共消费豆数为：'+rs.data.caution+'个</h1>',function(){
		...
		_validQuestion:function(){
			...
			//发送最后一题的答案
			if(answerStr == undefined){
				that._saveLastQuestionIO({answer: '-1'},function(rs){
					if(rs.code == 0){
						that._saveLastQuestion();
					}else{
				...
				answerVal = parseInt(answerStr) + 65;
				answer = String.fromCharCode(answerVal);
				that._saveLastQuestionIO({answer: answer},function(rs){
					if(rs.code == 0){
						that._saveLastQuestion();
					}else{
						PW.page.Util.dialog.alert('<h1>'+rs.errMsg +'</h1>');
		...
		_getQuestion:function(currentIndex, nextIndex, answerVal){
			var
				that = this,
				answer,
				data = {previousIndex: currentIndex,nextIndex: nextIndex, answer:answerVal};
...
/*--------------------------------退出------------------------------*/
KISSY.add('groupExamTest/exit',function(S){
	var
...
/*--------------------------------点击非白色地方报错------------------------------*/
KISSY.add('groupExamTest/click',function(S){
	var
...
/**************************答题计时器*****************************************/
KISSY.add('groupExamTest/timer',function(S){
	var
		$ = S.all, DOM = S.DOM, on = S.Event.on,
		el = {
			//答题计时
...
/*********************将页面导航页面操作加入到页面****************************/
KISSY.add('groupExamTest/index',function(S){
	function index(){
		this._init();
	}
	S.augment(index,{
