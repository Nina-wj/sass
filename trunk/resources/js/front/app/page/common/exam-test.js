...
*     前台学习测试页面操作
* --------------------------------------------------------------------------*/
KISSY.add('page/common/exam-test',function(S,get,exit,click,index){
	PW.namespace('page.common.ExamTest');
	PW.page.common.ExamTest = function(param,IO){
		new get(param,IO);
		new exit(param);
		new click(param);
		new index();
	...
	requires:['examTest/get','examTest/exit','examTest/click','examTest/index']
});
/*--------------------------------获取题目------------------------------*/
KISSY.add('examTest/get',function(S,Juicer){
	var
		...
		_getQuestion:function(){
			...
			that.ExamIO.getOneQuestion(data,function(rs){
				if(rs.code == 0){
					...
					DOM.val(el.questionId,rs.data.questionId);
				}else{
					PW.page.Util.dialog.alert('<h1>'+rs.errMsg+'</h1>');
				}
			});
...
/*--------------------------------退出------------------------------*/
KISSY.add('examTest/exit',function(S){
	var
...
/*--------------------------------点击非白色地方报错------------------------------*/
KISSY.add('examTest/click',function(S){
	var
...
/*********************将页面导航页面操作加入到页面****************************/
KISSY.add('examTest/index',function(S){
	function index(){
		this._init();
	}
	S.augment(index,{
